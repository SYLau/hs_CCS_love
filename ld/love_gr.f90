        ...
        integer::nold
            ...
            do
                ...
                if (adaptive) then
                    h=(tb-t)/500
                    hmin=(tb-t)*eps_hmin
                    call rk45ad(eq_chan15,t,x,h,tb,itmax,emax,iflag,.true.,hmin=hmin)
                else
                    call rk4ug(eq_chan15,t,x,ug_steps,tb,.true.,xsq)
                endif
                ...
                x=x + pi4*t**3/(m_i+pi4*t**3*p_i/c**2)*drho_i !Junction condition
                i=i+1
            enddo
            t=tovs(size(tovs))%r
            x=x-pi4*t**3/(tovs(size(tovs))%m)*tovs(size(tovs))%rho !Junction condition between surface and vacuum
